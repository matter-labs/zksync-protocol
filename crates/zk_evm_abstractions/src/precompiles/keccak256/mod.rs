...
// When compiling tests, we want legacy impl to be available to compare against.
...
cfg_if! {
    if #[cfg(feature = "airbender-precompile-delegations")] {
        use self::airbender_backend::DelegatedKeccakBackend as ActiveKeccakBackend;
    } else {
        use self::legacy_backend::LegacyKeccakBackend as ActiveKeccakBackend;
    }
}
