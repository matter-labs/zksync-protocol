...
// ==============================================================================
// Keccak Backend Selection
// ==============================================================================
//
// The precompile loop is identical for both implementations. The only backend-specific
// behavior is how bytes are accumulated between reads and how the final digest state is
// materialized. We encode that seam in a trait so the production path picks one backend,
// while tests can instantiate both and compare them through the same executor.
trait KeccakBackend {
    fn new() -> Self;
...
cfg_if! {
    if #[cfg(any(not(feature = "airbender-precompile-delegations"), test))] {
        struct LegacyKeccakBackend {
...
cfg_if! {
    if #[cfg(feature = "airbender-precompile-delegations")] {
        type ActiveKeccakBackend = DelegatedKeccakBackend;
    } else {
        type ActiveKeccakBackend = LegacyKeccakBackend;
    }
}

fn execute_keccak_precompile<M: Memory, Backend: KeccakBackend, const B: bool>(
    ...
    let mut round_accumulator = Backend::new();
    ...
    for round in 0..num_rounds {
        ...
        for idx in 0..MEMORY_READS_PER_CYCLE {
            ...
            let enough_buffer_space = round_accumulator.can_fill_bytes(meaningful_bytes_in_query);
            ...
            if should_read {
                ...
                let data_query = memory.execute_partial_query(monotonic_cycle_counter, data_query);
        ...
        round_accumulator.finish_round(&full_round_padding, is_last, paddings_round, padding_space);
...
impl<const B: bool> Precompile for Keccak256Precompile<B> {
    ...
    fn execute_precompile<M: Memory>(
        ...
        execute_keccak_precompile::<M, ActiveKeccakBackend, B>(
...
mod airbender_backend_tests {
    use super::{
        execute_keccak_precompile, DelegatedKeccakBackend, Keccak256RoundWitness,
        LegacyKeccakBackend,
    };
    ...
    fn make_query(input_offset: u32, input_length: u32) -> LogQuery {
    ...
    fn run_precompile<Backend: super::KeccakBackend>(
        input_offset: u32,
        input: &[u8],
    ) -> KeccakExecution {
        let query = make_query(input_offset, input.len() as u32);
        let mut memory = InputMemory::for_input(input_offset, input);
        execute_keccak_precompile::<InputMemory, Backend, true>(0, query, &mut memory)
    }

    #[test]
    fn keccak256_delegated_backend_matches_legacy_backend() {
        let vectors = [
            (0u32, 0u32),
            (0, 1),
            (0, 2),
        ...
        for (input_offset, input_length) in vectors {
            let input = (0..input_length as usize)
                .map(|idx| {
                    ((idx as u8).wrapping_mul(37))
                        .wrapping_add((input_offset as u8).wrapping_mul(11))
                        .wrapping_add(input_length as u8)
                })
                .collect::<Vec<_>>();
            let legacy = run_precompile::<LegacyKeccakBackend>(input_offset, &input);
            let delegated = run_precompile::<DelegatedKeccakBackend>(input_offset, &input);

            assert_eq!(
                delegated, legacy,
                "delegated backend must match legacy backend for offset={input_offset}, length={input_length}",
            );
        }
    }
}
