...
>(
    ...
    ecadd_memory_states: Vec<MemoryQueueState<F>>,
    ecadd_witnesses: Vec<(u32, LogQuery_, ECAddRoundWitness)>,
    ecadd_queries: Vec<LogQuery_>,
    mut demuxed_ecadd_queue: LogQueueStates<F>,
    num_rounds_per_circuit: usize,
    round_function: &R,
) -> (Vec<EcAddCircuitInstanceWitness<F>>, usize) {
    ...
    let amount_ecadd_memory_queries = ecadd_memory_queries.len();
    ...
    let amount_of_memory_queries_after =
        amount_of_memory_queries_before + amount_ecadd_memory_queries;
