...
pub(crate) fn ecmultipairing_naive_memory_queries(
    ...
    for (_cycle, _query, witness) in ecmultipairing_witnesses.iter() {
        let initial_memory_len = ecmultipairing_naive_memory_queries.len();
...
pub(crate) fn ecmultipairing_naive_decompose_into_per_circuit_witness<
    F: SmallField,
...
>(
    ...
    mut demuxed_ecmultipairing_naive_queue: LogQueueStates<F>,
    ...
    assert!(precompile_calls.len() == demuxed_ecmultipairing_naive_queue.states_accumulator.len());
    drop(demuxed_ecmultipairing_naive_queue.states_accumulator);
    ...
    let mut memory_queue_states_it = ecmultipairing_naive_memory_states.iter();
    ...
    for (request_idx, (request, per_request_work)) in precompile_calls
        ...
        .enumerate()
    {
        let _ = demuxed_ecmultipairing_naive_queue
            .simulator
        ...
        assert_eq!(amount_of_queries, 20);
        round_counter += 1;
        ...
        if round_counter == num_rounds_per_circuit || is_last_request {
            ...
            let witness = EcMultiPairingCircuitInstanceWitness::<F> {
                closed_form_input: EcMultiPairingCircuitInputOutputWitness::<F> {
                    ...
                    hidden_fsm_input: EcMultiPairingCircuitFSMInputOutputWitness::<F> {
                        log_queue_state: log_queue_input_state.clone(),
                        memory_queue_state: memory_queue_input_state,
                    },
                    hidden_fsm_output: EcMultiPairingCircuitFSMInputOutputWitness::<F> {
