...
use crate::zkevm_circuits::bn254::ec_mul::input::{
    EcMulCircuitFSMInputOutputWitness, EcMulCircuitInputOutputWitness, EcMulCircuitInstanceWitness,
};
use circuit_definitions::encodings::*;
use circuit_definitions::zk_evm::zk_evm_abstractions::precompiles::ecmul::ECMulRoundWitness;

pub(crate) fn ecmul_memory_queries(
    ecmul_witnesses: &Vec<(u32, LogQuery_, ECMulRoundWitness)>,
) -> Vec<MemoryQuery> {
    let amount_of_queries = ecmul_witnesses.iter().fold(0, |inner, (_, _, witness)| {
        inner + witness.reads.len() + witness.writes.len()
    });

    let mut ecmul_memory_queries = Vec::with_capacity(amount_of_queries);

    for (_cycle, _query, witness) in ecmul_witnesses.iter() {
        let initial_memory_len = ecmul_memory_queries.len();

        // we read, then write
        ecmul_memory_queries.extend_from_slice(&witness.reads);
        ecmul_memory_queries.extend_from_slice(&witness.writes);

        assert_eq!(ecmul_memory_queries.len() - initial_memory_len, 6);
    }
...
pub(crate) fn ecmul_decompose_into_per_circuit_witness<
    F: SmallField,
    R: BuildableCircuitRoundFunction<F, 8, 12, 4> + AlgebraicRoundFunction<F, 8, 12, 4>,
>(
    amount_of_memory_queries_before: usize,
    ecmul_memory_queries: Vec<MemoryQuery>,
    ecmul_simulator_snapshots: Vec<SimulatorSnapshot<F, FULL_SPONGE_QUEUE_STATE_WIDTH>>,
    ecmul_memory_states: Vec<QueueStateWitness<F, FULL_SPONGE_QUEUE_STATE_WIDTH>>,
    ecmul_witnesses: Vec<(u32, LogQuery_, ECMulRoundWitness)>,
    ecmul_queries: Vec<LogQuery_>,
    mut demuxed_ecmul_queue: LogQueueStates<F>,
    num_rounds_per_circuit: usize,
    round_function: &R,
) -> (Vec<EcMulCircuitInstanceWitness<F>>, usize) {
    assert_eq!(ecmul_memory_queries.len(), ecmul_memory_states.len());

    let memory_simulator_before = &ecmul_simulator_snapshots[0];
    assert_eq!(
    ...
    let precompile_calls = ecmul_queries;
    let simulator_witness: Vec<_> = demuxed_ecmul_queue.simulator.witness.clone().into();
    let round_function_witness = ecmul_witnesses;

    // check basic consistency
    assert!(precompile_calls.len() == demuxed_ecmul_queue.states_accumulator.len());
    drop(demuxed_ecmul_queue.states_accumulator);
    ...
    // convension
    let mut log_queue_input_state = take_queue_state_from_simulator(&demuxed_ecmul_queue.simulator);
    let amount_ecmul_memory_queries = ecmul_memory_queries.len();
    let mut memory_queries_it = ecmul_memory_queries.into_iter();
    ...
    let mut memory_queue_states_it = ecmul_memory_states.iter();
        ...
        .enumerate()
    {
        ...
        if round_counter == num_rounds_per_circuit || is_last_request {
            ...
            log_queue_input_state = take_queue_state_from_simulator(&demuxed_ecmul_queue.simulator);
    ...
    let memory_simulator_after = &ecmul_simulator_snapshots[1];
    let amount_of_memory_queries_after =
        amount_of_memory_queries_before + amount_ecmul_memory_queries;
