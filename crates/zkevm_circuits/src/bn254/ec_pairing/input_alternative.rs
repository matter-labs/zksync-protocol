...
pub struct EcMultiPairingFunctionFSM<F: SmallField> {
    ...
    pub precompile_call_params: EcMultiPairingPrecompileCallParams<F>,
}

impl<F: SmallField> CircuitVarLengthEncodable<F> for EcMultiPairingFunctionFSM<F> {
    #[inline(always)]
...
impl<F: SmallField> WitnessVarLengthEncodable<F> for EcMultiPairingFunctionFSM<F> {
    fn witness_encoding_length(witness: &Self::Witness) -> usize {
        ...
        total_len += <EcMultiPairingPrecompileCallParams<F> as WitnessVarLengthEncodable<F>>::witness_encoding_length(&witness. precompile_call_params);
        total_len
    }
    fn encode_witness_to_buffer(witness: &Self::Witness, dst: &mut Vec<F>) {
        ...
        <EcMultiPairingPrecompileCallParams<F> as WitnessVarLengthEncodable<F>>::encode_witness_to_buffer(&witness. precompile_call_params, dst);
    }
}

impl<F: SmallField> CSPlaceholder<F> for EcMultiPairingFunctionFSM<F> {
    fn placeholder<CS: ConstraintSystem<F>>(cs: &mut CS) -> Self {
        ...
        Self {
            ...
            precompile_call_params: EcMultiPairingPrecompileCallParams::<F>::placeholder(cs),
        }
    }
}
...
pub struct EcMultiPairingCircuitFSMInputOutput<F: SmallField> {
    pub internal_fsm: EcMultiPairingFunctionFSM<F>,
    ...
    F: SmallField,
{
    ...
    {
        Self {
            internal_fsm: EcMultiPairingFunctionFSM::placeholder(cs),
