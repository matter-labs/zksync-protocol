...
use boojum::pairing::bn256::{Fq, Fq12, Fq2, Fq6};
use std::sync::Arc;
...
#[derive(Clone, Debug)]
pub struct TorusWrapper<F: SmallField> {
    encoding: Fp6<F>,
}

impl<F: SmallField> TorusWrapper<F> {
    pub fn get_params(&self) -> &Arc<RnsParams> {
    ...
    pub fn replace_by_constant_if_trivial<CS: ConstraintSystem<F>>(
        ...
        let allocated_cnst = Fp6::allocate_constant(cs, compressed_cnst);
        let new_encoding = Fp6::conditionally_select(
            cs, &is_trivial, &compressed_cnst, &self.encoding
        );
    ...
    pub fn new(encoding: Fp6<F>) -> Self {
        let res = TorusWrapper { encoding };
        res
    }
